Attribute VB_Name = "sw14_func_create_A3"
    
'工程图模板所在路径
Const strFormatePath As String = _
"C:\ProgramData\SOLIDWORKS\SOLIDWORKS 2019\lang\english\sheetformat\A3.drwdot"
'BOM的模板
...
'    Dim swApp As SldWorks.SldWorks
    ...
    'Create new drawing
    '{
    ' A1.drwdot+dblRatio1 代表 A4 ， A1.drwdot+dblRatio0.5 sw会卡死 ，A1.drwdot+dblRatio1 代表 A2 ，
    Dim dblPaperA1Length, dblPaperA1Width As Double
    dblPaperA3Length = 0.297
    dblPaperA3Width = 0.42
    Dim strTemplatePath As String
    strTemplatePath = swApp.GetUserPreferenceStringValue(swUserPreferenceStringValue_e.swDefaultTemplateDrawing)
    Set swModel = swApp.NewDocument(strFormatePath, swDwgPapersUserDefined, dblPaperA3Width, dblPaperA3Length)
    '}
    
    Set swDraw = swModel
    
    'Setup first sheet
    swDraw.SetupSheet5 "Sheet1", 8, 8, 1, 4, True, strTemplatePath, dblPaperA3Width, dblPaperA3Length, "Default", True
    
    'Add a new sheet
'    swDraw.NewSheet3 "Sheet1", 12, 12, 1, 4, False, strFormatePath, dblPaperA1Width, dblPaperA1Length, "Default"
    
    'Change name of shets
    ...
    For i = 0 To UBound(vSheetNames)
        ...
        If i = 0 Then swSheet.SetName "Full Assy"
        If i = 1 Then swSheet.SetName "Handle"
    Next i
    ...
    Set swView = swDraw.CreateDrawViewFromModelView3(strModelPath, "*Isometric", 0.7 / 2.05, 0.45 / 2.05, 0)
...
'        Set swTransform = swTransform.Inverse
    ...
    dblX1 = Round(vSheetsProps(3) * (vPts(0) - 0.05), 3)
    dblY1 = Round(vSheetsProps(3) * vPts(1), 3)
    dblX2 = Round(vSheetsProps(3) * (vPts(0) + 0.05), 3)
